...
public class Utilities
{
    ...
    private static float DistanceBetweenPoints(Points p1, Points p2)
    {
        double x_1 = p1.X;
        double y_1 = p1.Y;
        double x_2 = p2.X;
    ...
    private static (float, double) Rectangulares_Polares(float x, float y) // Cambiar nombre!!!!!!!
    {
        double rho = Math.Sqrt(Math.Pow(x, 2) + Math.Pow(y, 2));
        double angle = Math.Atan(y / x);
        angle = (angle / Math.PI) * 180;

        return ((float)rho, angle);
    }
